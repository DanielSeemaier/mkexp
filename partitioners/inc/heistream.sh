...
Fetch() {
    ...
    if (( ${fetch_args[install_kagen_driver]} )); then 
        echo "KaGen support is not implemented for HeiStream"
        exit 1
    fi
}
...
FetchDiskDriver() {
    ...
    GenericGitFetch fetch_disk_driver_args "$HEISTREAM_REPOSITORY_URL" "disk_driver_src"
}
...
InstallDiskDriver() {
    ...
    Prefixed cmake --build "$src_dir/build" --target heistream --parallel
    Prefixed cp "$src_dir/build/heistream" "${install_disk_driver_args[disk_driver_bin]}"
}
